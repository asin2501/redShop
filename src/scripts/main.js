...
$(document).ready(function() {
	...
	function redirectFromSearch() {
		if($('.bc-sf-search-container').length) {
			$('body').on('click', '.product-card__image-link', function(){
				localStorage.setItem('srf', 'true');
			});
		}

		if($('.product').length) {
			var redirectFlag = localStorage.getItem('srf');

			if(redirectFlag === 'true') {
				$('.js-return-link').attr('href', '/search');
				localStorage.removeItem('srf');
			}
		}
	}
	...
	function initLoginForm() {
		if ($('.login-page').length) {
			new Vue({
				...
				data:  {
					loginShown: true,
					forgottPassShown: false
				},
				methods: {
					changeView: function(){
						this.loginShown =  !this.loginShown;
						this.forgottPassShown = !this.forgottPassShown;
					}
				}
			});
		}
